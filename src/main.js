...
function render() {
  ...
  document.querySelector('#app').innerHTML = `
    <header class="site-header">
      ...
      <nav class="desktop-nav" aria-label="主导航"><a href="#capabilities">定位</a><a href="#workflow">案例</a><a href="#architecture">架构</a><a href="#workers">Worker</a><a href="#ecosystem">资源</a><a href="#boundaries">边界</a></nav>
      ...
      <button class="mobile-menu" aria-label="打开菜单" aria-expanded="false" aria-controls="mobile-drawer">${icons.menu}</button>
    </header>
    <div class="mobile-drawer" id="mobile-drawer" aria-hidden="true"><button class="drawer-close" aria-label="关闭菜单">${icons.close}</button><a href="#capabilities">定位</a><a href="#workflow">案例</a><a href="#architecture">架构</a><a href="#workers">Worker</a><a href="#ecosystem">资源</a><a href="#boundaries">边界</a><a href="#closing">进入体验 ${icons.arrow}</a></div>
    <main>
      <section class="hero" id="top">
        <div class="hero-copy reveal"><p class="eyebrow"><span class="eyebrow-dot"></span> 本地优先 · 角色 Agent 运行时</p><h1>让角色，<em>不止于</em><br />对话。</h1><p class="hero-lede">YUMENO 把意图、知识、声音和工具编排成一条可以真正完成任务的 执行链路。</p><div class="hero-actions"><a class="button button-primary" href="#workflow">先看真实任务 ${icons.arrow}</a><a class="button button-ghost" href="#architecture">再看执行架构</a></div><div class="hero-note"><span>01</span><div><strong>对话即控制台</strong><p>从一句自然语言开始，让角色完成资源准备、文件处理和知识工作。</p></div></div></div>
...
function bindEvents() {
  ...
  document.querySelector('.mobile-menu')?.addEventListener('click', () => setDrawerOpen(true));
  document.querySelector('.drawer-close')?.addEventListener('click', () => setDrawerOpen(false));
  document.querySelectorAll('.mobile-drawer a').forEach((link) => link.addEventListener('click', () => setDrawerOpen(false)));
  bindAudio();
  bindLive2D();
  observeReveals();
}
